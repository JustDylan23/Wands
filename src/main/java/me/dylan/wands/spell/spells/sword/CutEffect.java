...
final class CutEffect {
    ...
    static void draw(@NotNull Player player, double degrees, double radius, int damage, int rotation, boolean fullCircle) {
        draw(player, degrees, radius, damage, rotation, fullCircle, null);
    }

    static void draw(@NotNull Player player, double degrees, double radius, int damage, int rotation, boolean fullCircle, @Nullable Vector offset) {
        ...
        world.playSound(location.clone().add(newVector(rotation, radius, rotX, rotY, rotZ)),
                Sound.ENTITY_WITHER_SHOOT, 2, (float) ThreadLocalRandom.current().nextDouble(1.6, 2));
